...
class Seat(models.Model):
    seat_number = models.PositiveSmallIntegerField(verbose_name="Номер місця")
    start_time = models.DateTimeField(verbose_name="Дата початку показу")
    end_time = models.DateTimeField(verbose_name="Дата закінчення показу")
    status = models.CharField(
        ...
        default='free',
        verbose_name="Статус місця"
    )

    def __str__(self):
        return f"Місце {self.seat_number} для фільму"

    class Meta:
        verbose_name = "Місце для показу"
        verbose_name_plural = "Місця для показу"
...
class Movie(models.Model):
    ...
    runtime = models.PositiveSmallIntegerField(default=0, verbose_name="Тривалість фільму", help_text="Тривалість у хвилинах")
    ...
    created_at = models.DateTimeField(default=timezone.now, verbose_name="Дата створення замрвлення:")

    seat = models.ManyToManyField(Seat, verbose_name="Час сенасу", blank=True, related_name="film_session")
...
class PaymentIntent(models.Model):
    ...
    movie_title = models.CharField(max_length=250, verbose_name="Заголовок фільму")
    seat_number = models.CharField(max_length=50, verbose_name="Номер місця")

    class Meta:
        verbose_name = "Причина платежу"
        verbose_name_plural = "Причини платежів"

class Payment(models.Model):
    first_name = models.CharField(max_length=250, verbose_name="Ім'я користувача")
    last_name = models.CharField(max_length=250, verbose_name="Прізвище користувача")
    email = models.EmailField(max_length=250, verbose_name="Пошта користувача")
    phone = models.CharField(max_length=250, verbose_name="Номер теелфону користувача")
    movie = models.ForeignKey(Movie, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Фільм")
    seat_number = models.PositiveSmallIntegerField(default=0, verbose_name="Номер місця")
